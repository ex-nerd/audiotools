...
def fix_album(s):
    ...
    if re.search(r"Dis[kc]\b", s, flags=re.IGNORECASE) or re.search(
        r"cd\s*\d", s, flags=re.IGNORECASE
    ):
        print(f"  WARNING, album with disk in name: {s}")
    return s
...
def fix_title(s):
    ...
    for term in (
        ...
        new = re.sub(r"( {0} )".format(term), _lowercase, new, flags=re.IGNORECASE)
        new = re.sub(
    ...
    new = re.sub(
        r"( \w(?: (?:flat|sharp))? )((?:major|minor))",
        _musiccase,
        new,
        flags=re.IGNORECASE,
    )
    new = re.sub(r"(?:#|No\.?)(\d)", r"No. \1", new)
    new = new.replace("\u2019", "'")  # smart apostrophe
    new = new.replace("`", "'")
    new = new.replace("\xb4", "'")  # some form of tilde meant to be apostrophe
    new = re.sub(r"\s+", " ", new)
    new = re.sub(r"\s+,", ",", new)
...
def fix_name(s):
    new = strip_accents(s)
    new = new.replace("\xa1", "")  # upside down !
    new = new.replace("\xe6", "ae")
    new = new.replace("\xdf", "ss")  # eszet
    new = new.replace("\xa1", "")  # upside down !
    new = new.replace("\xbf", "")  # upside down ?
    new = fix_title(new)
    new = new.replace('"', "")
...
def load_mp4(dir, file, _numtracks=None, _disk=None, _numdisks=None):
    ...
    if _disk and _numdisks and not disk and not numdisks:
        ...
        print(f"  Disknum mismatch {disk} != {_disk} on {file}")
        sys.exit(1)
    elif disk and not numdisks:
        print(f"  disknum but no numdisks on {file}")
        sys.exit(1)
    ...
    title = title.replace(
        ",,,", ";"
    )  # catch some oddities left over from fixing nero's tag issues
    ...
    # Generate a new name
    new = fix_name_full(
        "{0} {1}.m4a".format(str(track).zfill(max(2, len(str(numtracks)))), title)
    )
    if disk > 0 and numdisks > 1:
        new = "{0}-{1}".format(str(disk).zfill(len(str(numdisks))), new)
    try:
        if file.decode("utf-8") == new.decode("utf-8"):
            new = None
    except:
        print(f"  FILE:  {os.path.join(dirname, file)}")
        raise
    # Return
...
def load_mp3(dir, file, _numtracks=None, _disk=None, _numdisks=None):
    ...
    # Generate a new name
    new = fix_name_full(
        "{0} {1}.mp3".format(str(track).zfill(max(2, len(str(numtracks)))), title)
    )
    ...
    try:
        ...
        print(f"  FILE:  {os.path.join(dir, file)}")
        raise
    # Return
    return (m, new, changed)
...
def visit(arg, dirname, names):
        ...
        if file.startswith("."):
            ...
            try:
                ...
                if file.decode("utf-8") != new.decode("utf-8"):
                    ...
                    if not dryrun:
                        os.rename(
                            os.path.join(dirname, file), os.path.join(dirname, new)
                        )
            except:
                print(f"  FILE:  {os.path.join(dirname, file)}")
                raise
    ...
    for file in m4a:
        ...
        (mp4, name, changed) = load_mp4(
            dirname, file, len(m4a), int(disknum), int(numdisks)
        )
    ...
    for group in music:
        ...
        if "mp4" in group:
            ...
            if is_comp and not compilation:
                ...
                changed.append("partofcompilation")
            if compilation and not (
                t3["albumartist"] in m and m[t3["albumartist"]].text[0]
            ):
